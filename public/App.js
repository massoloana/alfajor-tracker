...
function AlfajorCard({ alfajor, rank }) {
  return (
      ...
      <div className="bg-white rounded-[1.5rem] shadow-2xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-3">
          ...
          <button onClick={submit} disabled={loading}
            ...
            {loading ? 'Guardando...' : 'Guardar Alfajor'}
          </button>
        </div>
      </div>
    </div>
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-2 md:grid-cols-3">
            {filtrados.map((a, i) => (
              <AlfajorCard key={a.id} alfajor={a} rank={i + 1} />
            ))}
          </div>
        )}
      </div>

      {showModal && <AdminModal onClose={() => setShowModal(false)} onSaved={fetchAlfajores} />}
    </div>
  )
}
